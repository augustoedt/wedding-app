...
> **Atualizado em 2026-09-17.** Este é o checkpoint autoritativo: ao
...
| `wedding-app-front` (`444e994e-e77e-4ea3-9187-5f818068c09c`) | `/wedding-app-front` | `layout-v2` | `eduardoemayra.com.br` e `wedding-app-front-production.up.railway.app` |
...
- Documentação do monorepo sendo consolidada em `docs/` (README raiz + CLAUDE.md). Nada de docs de produto dentro dos três serviços.
- Branch local `layout-v2` à frente de `origin/layout-v2` só se houver commits novos após `8259698`. `main` local pode estar atrás de `origin/main` (não usar `git pull` sem conferir).
...
1. Fechar esta organização de docs (commit só de documentação).
2. Espelhar o commit de docs em `main` e `layout-v2` (docs na raiz não disparam build útil, mas o histórico fica alinhado).
3. Corrigir `docs/contracts/api.md` (24h → 7 dias) quando for conveniente — issue aberta.
4. Não subir Better Auth para 1.7 sem o plano `migrar-better-auth-1-7.md`.
...
- **Front no ar vem de `layout-v2`, API/admin de `main`.** Commit só em uma branch não atualiza o outro lado. ADR `003`.
