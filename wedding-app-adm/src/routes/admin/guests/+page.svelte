					...
					{#each guests.current as g (g.id)}
						<tr class="hover:bg-slate-50">
							...
							<td class="px-4 py-3">
								{#if g.plusOne === 0}
									—
								{:else if g.rsvp === 'confirmed'}
									{g.confirmedCompanions}/{g.plusOne}
								{:else}
									{g.plusOne}
								{/if}
							</td>
							...
							<td class="px-4 py-3">
								<select
									value={g.rsvp}
									onchange={(e) =>
										changeRsvp(g, (e.target as HTMLSelectElement).value as RsvpStatus)}
									class="cursor-pointer rounded-full border-0 px-2 py-1 text-xs font-medium focus:ring-1 focus:ring-rose-400 {rsvpColors[
										g.rsvp
									]}"
								>
							...
							<td class="px-4 py-3 text-right">
								<div class="inline-flex items-center gap-1">
									{#if g.phone && data.wedding?.siteUrl}
										<Tooltip
											text={g.inviteSent ? 'Reenviar convite' : 'Enviar convite via WhatsApp'}
										>
											<button
												onclick={() => sendInvite(g)}
												aria-label={g.inviteSent ? 'Reenviar convite' : 'Enviar convite'}
												class="p-1 {g.inviteSent
									...
									<Tooltip text="Editar">
										<a
											...
											aria-label="Editar"
											class="inline-flex p-1 text-slate-400 hover:text-slate-700"
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
												class="h-4 w-4"
